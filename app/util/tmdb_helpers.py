...
DESIRED_KEYS = ["title", "overview", "genres", "status", "release_date"]

DESIRED_KEYS = ["title", "overview", "genres", "status"]
...
def _format_tmdb_movie(tmdb_movie):
    formatted_movie = {
        desired_key: tmdb_movie.get(desired_key, None) for desired_key in DESIRED_KEYS
    }
    ...
    formatted_movie["posterPath"] = (
        "https://image.tmdb.org/t/p/w370_and_h556_bestv2/{}".format(poster_path)
        if poster_path
        else None
    )
...
def search_movies(search_term):
    params = {"api_key": TMDB_API_KEY, "query": search_term}
    response = requests.get(TMDB_SEARCH_MOVIE_URL, params=params)
    if response.status_code != 200:
        raise APIException(response.status_code, response.json()["status_message"])

...
class RecommendationController:
    def handle(self):
        ...
        positive_movie_ids = (
            set.intersection(*positive_movie_id_sets)
            if positive_movie_id_sets
            else set()
        )
        negative_movie_ids = (
            set.union(*negative_movie_id_sets) if negative_movie_id_sets else set()
        )
        recommended_movie_ids = list(positive_movie_ids.difference(negative_movie_ids))[
            :5

...
class MovieController:
    def search_movies(self):
        ...
        return json.dumps({"movies": tmdb_helpers.search_movies(searchword)})

    def explore_movies(self):
        sort_method = request.args.get("sort", None)

        return json.dumps({"movies": tmdb_helpers.explore(sort_method)})

    def list_liked_movies(self, user):
        user_movie_preferences = [
            mp
            for mp in user.movie_preferences
